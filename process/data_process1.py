    ...
    for dataset in os.listdir(datapath):
        ...
        for data in os.listdir(data_dir):
            ...
            for idx in range(0, len(y_enf)):
                ...
                reference = dataname + "_" + str(idx) + " " + lab.loc[lab.loc[:, 0] == dataname, 1].astype(
                    str) + " " + save_dir
    ...
    for dataset in os.listdir(datapath):
        ...
        for data in os.listdir(data_dir):
            ...
            for idx in range(0, len(y_enf)):
                ...
                # reference = dataname + "_" + str(idx) + " " + str(label) + " " + save_dir
                reference = dataname + "_" + str(idx) + " " + " " + save_dir

                write_to_txt(os.path.join(FEATPATH, 'train1.txt'), reference)


def write_to_txt(filepath, reference, mode="a+"):
...
def train_split(filedir=os.path.join(FEATPATH, 'train1.txt'), ratio=0.3):
    ...
    test = shuffle(test)
    test, val = train_test_split(test, test_size=0.333333)

    np.savetxt(os.path.join(FEATPATH, 'train5.txt'), train, delimiter=",", fmt='%s')  # delimiter=“，” 才能一个数据一个格
    np.savetxt(os.path.join(FEATPATH, 'test5.txt'), test, delimiter=",", fmt='%s')
    np.savetxt(os.path.join(FEATPATH, 'validation5.txt'), val, delimiter=",", fmt='%s')
