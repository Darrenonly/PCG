...
class MyBlock(nn.Module):
    def __init__(self, in_planes, growth_rate, stride=1):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(in_planes, mid_planes, 1, 1, 0, bias=False),
            nn.BatchNorm1d(mid_planes),
            # nn.ReLU(True),
            h_swish(),
            nn.Conv1d(mid_planes, growth_rate, 3, stride, 1, bias=False),
            # nn.BatchNorm1d(mid_planes),
            # nn.Conv1d(mid_planes, growth_rate, 1, 1, 0, bias=False),
        ...
        self.conv2 = nn.Sequential(
            nn.Conv1d(in_planes, growth_rate, 3, stride,0, bias=False),
            # nn.BatchNorm1d(mid_planes),
            # # nn.ReLU(True),
            # nn.Conv1d(mid_planes, growth_rate, 1, 1,0, bias=False),
            nn.BatchNorm1d(growth_rate),
...
def densenet_cifar_without_any():
    return DenseNet(MyBlock, [4,4,4,4], [1,1,2,1],growth_rate=12)

def test():
    from torchsummary import summary
    net = densenet_cifar_without_any()
    # print(net)
