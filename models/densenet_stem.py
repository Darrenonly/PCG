...
class MyBlock(nn.Module, ABC):
    def __init__(self, in_planes, growth_rate, stride=1):
        super(MyBlock, self).__init__()
        self.stride = stride
        mid_planes = int(growth_rate / 4)
        self.conv1 = nn.Sequential(
            nn.Conv1d(in_planes, mid_planes, 1, 1, 0, bias=False, groups=1),
            nn.BatchNorm1d(mid_planes),
            # nn.ReLU(True),
            h_swish(),
            nn.Conv1d(mid_planes, mid_planes, 3, stride, 1, bias=False, groups=mid_planes),
            nn.BatchNorm1d(mid_planes),
            nn.Conv1d(mid_planes, growth_rate, 1, 1, 0, bias=False, groups=1),
        ...
        self.conv2 = nn.Sequential(
            nn.Conv1d(in_planes, mid_planes, 3, stride, 1, bias=False, groups=mid_planes),
            nn.BatchNorm1d(mid_planes),
            # nn.ReLU(True),
            nn.Conv1d(mid_planes, growth_rate, 1, 1, 0, bias=False, groups=1),
            nn.BatchNorm1d(growth_rate),
...
class Channel_Attention(nn.Module, ABC):

    def __init__(self, channel, r):
        super(Channel_Attention, self).__init__()

        self.__avg_pool = nn.AdaptiveAvgPool1d(1)
        self.__max_pool = nn.AdaptiveMaxPool1d(1)

        self.__fc = nn.Sequential(
            nn.Conv1d(channel, channel // r, 1, bias=False),
            nn.ReLU(True),
            nn.Conv1d(channel // r, channel, 1, bias=False),
            nn.Dropout(0.3)
        )
...
class Spartial_Attention(nn.Module, ABC):
...
class Transition(nn.Module, ABC):
    def __init__(self, in_planes, out_planes):
        ...
        self.conv = nn.Sequential(
            BasicConv2d(in_planes, out_planes, activation=False, kernel_size=1),
            # nn.Conv1d(in_planes, out_planes, kern/el_size=1)
...
def densenet_cifar():
    return DenseNet(MyBlock, [4, 4, 4, 4], [1, 1, 2, 1], growth_rate=12)
